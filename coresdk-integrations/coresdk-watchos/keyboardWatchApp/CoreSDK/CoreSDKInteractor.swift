...
class CoreSDKInteractor {
    ...
    init?(layout: KeyboardLayout) {
        ...
        guard let fleksyLib = try? FleksyLib(languageFileURL: CoreSDKInteractor.enUS_FileURL, configuration: libraryConfiguration) else {
            return nil
        }
